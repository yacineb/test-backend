...
*utilisateur* ne doit pas détruire ce que son organisation possède encore.
L'action manquante force à trancher explicitement le jour où un chemin de
suppression d'utilisateur apparaîtra.

Trois propriétés portent le reste.

**`storage_key` est générée côté serveur : `{org_id}/{document_id}`.** Aucune
partie ne vient du client ; `filename` est stocké comme *donnée*, jamais utilisé
pour construire un chemin. Ce n'est pas de la défense en profondeur posée sur de
la validation, ça la remplace : ni `../` à rejeter, ni octet nul à retirer, ni
normalisation Unicode, ni collision de système de fichiers insensible à la casse,
parce qu'aucune chaîne contrôlée par l'utilisateur n'atteint un chemin. Un
assainisseur s'oublie sur un nouvel appelant ; un format de clé sans endroit où
mettre l'entrée d'un attaquant ne s'oublie pas. Le préfixe `org_id` est aussi la
frontière des règles de cycle de vie S3, des policies de bucket et d'une clé KMS
par tenant — se tromper plus tard est une migration de données.

**Les octets sont commités avant l'insertion de la ligne.** L'alternative
courante — insérer `status='uploading'` puis basculer — fait de « une ligne
existe » et « l'objet existe » deux faits distincts, donc chaque lecteur du
système, pour toujours, doit savoir quelles lignes sont réelles. Avec les octets
d'abord, il y a un invariant total : **toute ligne de `documents` référence un
objet complet et durable.** Le coût est qu'un crash entre le commit du stockage
et l'`INSERT` laisse un orphelin — une facture de stockage, invisible pour tous
les lecteurs, récupérable par un sweeper. Le mode de panne de l'alternative est
un chemin de lecture corrompu.

**`status` est une colonne, pas une décision de schéma repoussée.** Elle ne
contenait que `uploaded` à sa création, mais les états du pipeline y
atterrissent juste après, et l'ajouter plus tard aurait été une migration sur une
table déjà pleine.
...
D'où le fait que `put` possède tout le cycle write-commit-abort plutôt que de
rendre un handle : une interface à handle reporterait l'ordre du commit sur
chaque appelant et n'a aucune implémentation S3 correcte, puisque S3 n'a pas de
handle à donner. Conséquence : un upload crashé est *invisible* plutôt que
simplement bien rangé.

Le backend POSIX n'est pas un jouet : même contrat, avec les E/S bloquantes
déportées via `anyio.to_thread`. Ce point compte — `open()`/`write()` directement
dans un handler async bloque la boucle d'événements pendant l'écriture disque et
sérialise toutes les autres requêtes du worker, health checks compris. Parfait en
développement, à terre sous charge.

**Ce que l'interface n'a délibérément pas :** ni `exists()`, ni `list()`, ni
`copy()`, ni métadonnées, ni `presigned_url()`. Le dernier est le tentant (§3),
mais une méthode avec une implémentation, aucun appelant et aucun test n'est pas
de la préparation : c'est une hypothèse qui sera fausse sur un détail que seul le
second backend révélera.

## 3. Streamer à travers l'API, et quand arrêter

Les octets vont aujourd'hui client → API → stockage. L'alternative, ce sont les
URL présignées : le client `PUT` directement vers S3 et l'API ne touche jamais le
payload. C'est le design qui passe à l'échelle et c'est la direction ; la seule
question est de savoir s'il se justifie maintenant.

**L'arithmétique.** À la cible 12 mois, la pointe est à **2,8 uploads/s** ; à
2 Mo de PDF moyen, **5,6 Mo/s ≈ 45 Mbit/s**, avec environ **9 uploads en vol**,
chacun coûtant une tâche async et un fichier temporaire. Le design presigned
supprimerait une charge qui n'existe pas.

**Le déclencheur** n'est pas le nombre de documents mais la *taille moyenne des
objets*, et la relation est linéaire : avec 100 Mo de moyenne — un corpus de PDF
scannés — les mêmes 2,8 uploads/s deviennent **2,2 Gbit/s soutenus**, et le tier
d'API n'est plus dimensionné que pour pelleter des octets.
...
> chaque upload : la donnée pour trancher existe déjà.

**La migration sera peu chère**, parce que le presigned change où voyagent les
octets, pas le modèle de données : `storage_key` est déjà générée côté serveur
donc c'est déjà ce qui serait signé, « octets avant la ligne » correspond déjà au
cycle de vie presigned, le contrat `ObjectStore` est exactement ce que fournit
`CompleteMultipartUpload`, et le plafond de 100 Mo devient une condition
`content-length-range` de policy S3 — un point d'application plus strict, pas
plus faible. Ce qui s'ajoute vraiment, c'est le problème des uploads abandonnés :
raison de ne pas le prendre avant que la charge ne le justifie.

**Le coût connu d'aujourd'hui :** `UploadFile` fait spouler à Starlette le corps
dans un fichier temporaire avant le handler, donc sur le backend POSIX chaque
upload est écrit deux fois sur le disque local. Vraie inefficacité, pas le goulot
à ces tailles, et l'alternative est du multipart parsé à la main sur
`request.stream()` pour un gain non mesurable.
...
- **Atomicité** — une exception en cours de flux ne laisse ni la clé ni un `.tmp`,
  et un insert échoué supprime l'objet qu'il aurait référencé.
- **Plafond** — niveau 1 : un `Content-Length` surdimensionné est rejeté sans lire
  le corps, et un corps chunked qui sous-déclare sa taille est quand même
  interrompu ; niveau 2 : exactement la limite passe, limite + 1 échoue.
- **Intégrité et hygiène de clé** — les octets relus ont bien le SHA-256
  enregistré, et un nom hostile (`../../etc/passwd`) apparaît dans `filename` et
  nulle part dans `storage_key`.
- **Le type vient des octets** — un PNG déclaré PDF donne `415` ; un PDF déclaré
  `octet-stream` est stocké en `application/pdf`.
- **Auth et tenant** — `401` sans token, `security` présent dans le schéma
  OpenAPI, et glisser un `org_id` en champ de formulaire ne change rien.
- **RLS couvre `documents`** (vrai Postgres) — un `SELECT` non filtré ne voit pas
  les lignes d'une autre organisation, un `INSERT` visant ailleurs est refusé par
  `WITH CHECK`, et une connexion sans `app.current_org_id` voit zéro ligne plutôt
  que toutes.
